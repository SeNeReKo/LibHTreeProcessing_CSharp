...
namespace LibHTreeProcessing.src.transformation2.selectors
{
	...
	public class SelectSingleAttribute_Selector : AbstractSelector
	{
		...
		public SelectSingleAttribute_Selector(int lineNo, HExpression expression)
			: base(lineNo)
		{
		...
		public override void Process(TransformationRuleContext ctx, ScriptRuntimeContext dataCtx, AbstractFilter filter, AbstractOperation operation)
		{
			...
			if (filter != null) {
				data = (HAttribute)(filter.Process(ctx, dataCtx, currentPath, data));
				if (data == null) {
